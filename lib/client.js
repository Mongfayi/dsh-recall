...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			"action": "撤回本轮对话",
			"confirmTurn": "撤回本轮对话及其后的全部内容？已产生的代码/文件改动不会回退。",
			"notice": "已撤回的消息",
		...
		const en = {
			...
			"confirmTurn": "Recall this turn and everything after it? Any code or file changes it produced will NOT be reverted.",
		...
		function UserRecallNodeView(props) {
			...
			return react.createElement("div", { className: "dsr-user-row", "data-time-hover-root": true },
				...
				react.createElement(RecallControl, {
					...
					doRecall: () => recallRequest({ sessionId, boundary }),
